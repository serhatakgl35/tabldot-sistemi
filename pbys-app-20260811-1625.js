const APP_KEY = 'pbys_production_v10';
...
function createEmptyDB() {
  return {
    users: [], mealSelections: {}, expenses: [], payments: [], debts: [], leaveRequests: [], leavePreferences: [],
    leavePlanResults: [], laundry: [], laundryFaults: [], attendance: [], auditLogs: [], settings: { ...DEFAULT_DATA.settings, systemName: 'PBYS' }
  };
}
function loadDB() {
  try {
...
function normalizeState(data) {
  ...
  data.attendance ||= [];
  data.auditLogs ||= [];
  data.settings = { ...DEFAULT_DATA.settings, systemName: 'PBYS', ...(data.settings || {}) };
  ...
  data.users.forEach(u => {
    // Eski manager kayıtları güvenli biçimde Personel rolüne düşürülür.
...
async function refreshFromCloud(rerender = true) {
  if (!currentUser || !window.FirebaseBridge) return;
  try {
    setCloudStatus('', 'Buluttan yenileniyor');
    const nextState = await window.FirebaseBridge.loadState(currentUser);
    applyCloudState(nextState, rerender);
...
function hasManagementPermission() { return !!currentUser && ['personnel.view','attendance.view','meal.manage','finance.manage','leave.view','leave.plan','reports.view'].some(permission => hasPermission(permission)); }
...
async function bootFirebase() {
  try {
    ...
    firebaseBooted = true;
    setCloudStatus('', 'Firebase kontrol ediliyor');
...
function renderDashboard() {
  ...
  const pendingLeaveCard = hasPermission('leave.view') && (pendingLeaveRows.length || isCommander()) ? `
    <div class="card commander-leave-card section-gap">
      <div class="card-header">
        <div><h3>⏳ İzin Talepleri</h3><p>${pendingLeaveRows.length ? 'Onay bekleyen izin talepleri öncelikli olarak burada gösterilir.' : 'Onay bekleyen izin talebi bulunmuyor.'}</p></div>
        <button class="btn btn-primary btn-sm" onclick="goPage('leave-management')">${pendingLeaveRows.length ? 'Tümünü Gör' : 'İzin Yönetimi'}</button>
      </div>
      ${pendingLeaveRows.length ? `<div class="table-wrap"><table><thead><tr><th>Personel</th><th>İzin türü</th><th>Tarih</th><th>Gün</th><th>İşlem</th></tr></thead><tbody>
      ...
      ${pendingLeaves > pendingLeaveRows.length ? `<div class="card-body"><small>+ ${pendingLeaves - pendingLeaveRows.length} başka bekleyen izin talebi var.</small></div>` : ''}` : `<div class="empty">Onay bekleyen izin talebi bulunmuyor.</div>`}
    </div>` : '';

  const todayAttendanceDate = toISO(new Date());
  const todayAttendance = dailyAttendanceStats(todayAttendanceDate);
  const dutyCount = (todayAttendance.duty || 0) + (todayAttendance.temporary_duty || 0) + (todayAttendance.course || 0);
  const restCount = (todayAttendance.medical || 0) + (todayAttendance.rest || 0);
  const otherLeaveCount = (todayAttendance.excuse_leave || 0) + (todayAttendance.road_leave || 0);
  const commanderAttendanceCard = isCommander() && hasPermission('attendance.view') ? `
    <div class="card commander-status-card section-gap">
      <div class="card-header">
        <div><h3>📋 Bugünkü Personel Durumu</h3><p>${formatDayDate(todayAttendanceDate)} · Görev, istirahat ve izin durumları anlık olarak gösterilir.</p></div>
        <button class="btn btn-primary btn-sm" onclick="attendanceDateCursor=parseISO('${todayAttendanceDate}');attendanceWeekCursor=startOfWeek(attendanceDateCursor);goPage('attendance-overview')">Yoklama Özeti</button>
      </div>
      <div class="card-body">
        <div class="commander-status-metrics">
          <div class="commander-status-metric"><span>👥 Toplam</span><strong>${todayAttendance.total || 0}</strong><small>aktif personel</small></div>
          <div class="commander-status-metric"><span>✅ Mevcut</span><strong>${todayAttendance.present || 0}</strong><small>karakolda mevcut</small></div>
          <div class="commander-status-metric"><span>📍 Görevli</span><strong>${dutyCount}</strong><small>görev / geçici görev / kurs</small></div>
          <div class="commander-status-metric"><span>🏥 İstirahat</span><strong>${restCount}</strong><small>raporlu veya nöbet istirahati</small></div>
          <div class="commander-status-metric"><span>🏖️ Yıllık İzin</span><strong>${todayAttendance.annual_leave || 0}</strong><small>onaylı yıllık izin</small></div>
          <div class="commander-status-metric"><span>📅 Diğer İzin</span><strong>${otherLeaveCount}</strong><small>mazeret / yol izni</small></div>
        </div>
        <div class="commander-status-detail">
          <div class="commander-status-detail-head"><strong>Personel bazında durum</strong><span>İsim, görev yeri ve açıklamalar</span></div>
          <div class="attendance-groups">${attendanceGroupHtml(todayAttendanceDate)}</div>
        </div>
      </div>
    </div>` : '';
  ...
  document.getElementById('pageContent').innerHTML = isCommander()
    ? pendingLeaveCard + commanderAttendanceCard + management + personal
    : personal + management + pendingLeaveCard;
}
...
function newMemberModal() {
  ...
  showModal('Yeni Personel Ekle', `<form id="newMemberForm" class="form-grid">
    ...
    <label class="span-2">Geçici şifre<input name="password" type="password" minlength="8" placeholder="En az 8 karakter" required></label>
...
function editMemberModal(userId) {
  ...
  showModal(`${user.name} · Personel Bilgileri`, `<form id="editMemberForm" class="form-grid">
    ...
    <label>Telefon<input name="phone" value="${escapeHtml(user.phone)}" readonly required><small class="form-note">Giriş kimliği olduğu için telefon numarası bu ekrandan değiştirilemez.</small></label>
...
function faultModal() {
  ...
  document.getElementById('faultForm').addEventListener('submit',e=>{e.preventDefault();const f=new FormData(e.target),machine=f.get('machine');db.laundryFaults||=[];db.laundryFaults.push({id:Date.now(),userId:currentUser.id,machine,note:f.get('note'),status:'Açık',createdAt:new Date().toISOString()});logAudit('laundry.fault',`${machine}: ${f.get('note')}`);saveDB();closeModal();renderLaundry();toast('Arıza kaydı oluşturuldu. Yetkili personel cihaz durumunu güncelleyecektir.');});
}
...
function reportHtml(type) {
  ...
  return `${head(`Aylık Tabldot Bilançosu · ${calc.label}`)}<div class="report-summary"><span>Toplam gider: <strong>${money(calc.totalExpense)}</strong></span><span>Toplam tabldot öğünü: <strong>${calc.totalMeals}</strong></span><span>Öğün birim maliyeti: <strong>${money(calc.unit)}</strong></span></div><h3>Giderler</h3><table><thead><tr><th>Tarih</th><th>Malzeme</th><th>Tutar</th></tr></thead><tbody>${expenseRows||'<tr><td colspan="3">Gider yok.</td></tr>'}</tbody></table><h3>Personel Hesabı</h3><table><thead><tr><th>Personel</th><th>Öğün</th><th>Tutar</th></tr></thead><tbody>${personRows}</tbody></table>`;
}
...
function exportBackup() { const blob = new Blob([JSON.stringify(db, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `pbys-firestore-yedek-${toISO(new Date())}.json`; a.click(); URL.revokeObjectURL(a.href); toast('Yedek dosyası indirildi.'); }
...
function openPasswordModal() {
  showModal('Şifre Değiştir', `<form id="passwordForm" class="form-grid"><label class="span-2">Yeni şifre<input name="password" type="password" minlength="8" required></label><label class="span-2">Yeni şifre tekrar<input name="confirm" type="password" minlength="8" required></label><div class="span-2"><button class="btn btn-primary btn-block">Şifreyi Güncelle</button></div></form>`);
